...
def main(gfa_regex, cog_threshold, output):
    ...
    for file in list_gfa_files:
        ...
        bin_ = re.findall(".*?Bin_.*?(Bin_.+?)/", file)[0]
    ...
    dict_to_flag2, dict_merge_bins2 = get_overlaping_bins(
        dict_cogbin_unitigs_merged, cog_threshold)
    if dict_merge_bins2:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "gfa_regex", help='expect a regular expression matching all cog simplified subgraphs of all bins, be advised that bins folders must explicitely start by "bin_".  example : subgraphs/Merged_bin_/bin_*/simplif/COG*.gfa')
    parser.add_argument(
