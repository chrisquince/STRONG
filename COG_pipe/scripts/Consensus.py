...
def main(Bin_file):
    ...
    number_of_dot = Counter(
        map(lambda x: len(x.split(".")), Dico_contig_bins.keys()))
    if len(number_of_dot) > 3:
        print("Error the number of dot in your contigs name is varying too much, it is not possible to distinguich cuts contigs from normal ones")
        exit(1)
    num_split = max(number_of_dot.keys())
    Dico_normal_contigs = {contig: bins for (
        contig, bins) in Dico_contig_bins.items() if len(contig.split(".")) != num_split}
    ...
    Dico_split = {contig: Counter(list_bins)
                  for contig, list_bins in Dico_split.items()}
    # Assign each contig to an unique bin
    Dico_splitcontig_bins = {contig: max(CounterBins.items(), key=lambda x: x[1])[
        0] for contig, CounterBins in Dico_split.items()}
    # Output results
    print("contig_id,0")
