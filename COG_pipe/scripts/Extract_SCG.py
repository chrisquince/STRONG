...
def main(gff_file, Annotation_file, SCG_file, faa_file, bed_file):
    def get_orf_name(ORF): return ORF.split(
        '\t')[0]+"_"+ORF.split('\t')[8].split(';')[0].split("_")[1]
    Set_scg = {line.rstrip() for line in open(SCG_file)}
    Dico_orfs_cogs = {"_".join(line.split("\t")[0].split("_")[:-1])+"_"+line.split("\t")[0].split(
        "_")[-1]: line.split("\t")[1] for line in open(Annotation_file) if line.split("\t")[1] in Set_scg}
    Bed_like = [[get_orf_name(ORF), ORF.split('\t')[3], ORF.split('\t')[4], Dico_orfs_cogs[get_orf_name(ORF)], ORF.split('\t')[
        6]] for _, _, ORF_list in prodigal_gff_parser(open(gff_file)) for ORF in ORF_list if get_orf_name(ORF) in Dico_orfs_cogs]
    ...
    for header, seq in SimpleFastaParser(open(faa_file)):
        ...
        if header in Dico_orfs_cogs:
            print(">"+header+" "+Dico_orf_strand[header][0] +
                  " strand="+Dico_orf_strand[header][1]+"\n"+seq)
