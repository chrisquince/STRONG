...
def main(fasta_file, bin_composition, set_bins, output, folder):
    Dico_contigs_bin = {line.rstrip().split(",")[0]: line.rstrip().split(",")[
        1] for line in open(bin_composition) if line.rstrip().split(",")[1] in set_bins}
    Dico_bin_Handle = {}
    for bins in set(Dico_contigs_bin.values()):
        if folder:
            os.system("mkdir -p "+output+"/Bin_"+bins)
        Dico_bin_Handle[bins] = open(
            output+("/Bin_"+bins+"/")*folder+"/Bin_"+bins+"."+fasta_file.split(".")[-1], "w")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "fasta_file", help="fasta file containing sequences binned")
    parser.add_argument(
        "bin_composition", help="csv file giving bin composition, output of concoct, first column=contig id, second is bin number")
    parser.add_argument(
        "path_output", help="specify the place you want all these bin folder to be put")
    parser.add_argument(
        "-l", nargs='+', help="restrict to a list of specifics bins ")
    parser.add_argument(
        "-d", help=" actually create a folder for each bin", action='store_true')
    ...
    List_all_bins = list({line.rstrip().split(
        ",")[1] for line in open(bin_composition)})
    Nb_bins = len(List_all_bins)
    nb_batch = Nb_bins//max_nb_handles+1
    List_Batchs = [List_all_bins[batch*max_nb_handles:min(
        (batch+1)*max_nb_handles, Nb_bins)] for batch in range(nb_batch)]
